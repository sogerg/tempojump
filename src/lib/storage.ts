...
export async function loadCustomChecklistItems(): Promise<CustomChecklistItem[]> {
  const raw = await AsyncStorage.getItem(CUSTOM_CHECKLIST_ITEMS_KEY);
  return raw ? (JSON.parse(raw) as CustomChecklistItem[]) : [];
}
